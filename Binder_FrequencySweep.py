# Title: Analysis of Frequency Sweep test results
# 
# Description: This script includes functions to perform the free shifting of the measured asphalt binder isotherms and 
#   construct the master curves, as well as fitting the master curve models to the shifted data. 
# 
# Example input: Input data should be entered as a DataFrame with at least 4 columns of:
#   - Temperature (°C)
...
# ======================================================================================================================
# === NOTE: ============================================================================================================
# ====== FOR THE LATEST VERSION OF THE FUNCTIONS IN THIS SCRIPT, PLEASE VISIT THE "AutoMC" REPOSITORY (GITHUB) =========
# ====== AT FHWA-ABML GITHUB PAGE: =====================================================================================
# ====== https://github.com/TFHRC-ABML =================================================================================
# ======================================================================================================================
# ======================================================================================================================
...
import numpy as np
import pandas as pd
from scipy.optimize import minimize_scalar, curve_fit, minimize, root_scalar
from scipy.interpolate import Akima1DInterpolator, interp1d


def Analyze_FrequencySweep_Data(df):
    """
    This function is an initializer to first perform the free shifting technique (developed by Gordon & Shaw 1994) and 
    then fit the CA, CAM, and 2S2P1D models to the data. 

    :param df: the main input dataframe including at least four columns for temperature, loading frequency, complex 
    shear modulus and phase angle. 
    :return: A dictionary of the results. 
    """
    # Prepare the dataFrame for shifting. 
    df.rename(columns={'Temperature (°C)': 'Temperature', 'Angular Frequency (rad/s)': 'Frequency', 
                       '|G*| (Pa)': '|G*|', 'Phase Angle (°)': 'PhaseAngle'}, inplace=True)
    df = df[['Temperature', 'Frequency', '|G*|', 'PhaseAngle']].copy()      # Keep only the required columns. 
    if 'IsOutlier' not in df.columns:
        df['IsOutlier'] = 0
    # First perform the Free shifting. 
    ShiftMapping, df = FreeShift_GordonShaw1994(df, 25, -30, {'Active': False})
    # Prepare the shifting coefficients as DataFrame. 
    ...
    ShiftCoeff = ShiftCoeff.sort_values(by='Temperature')
    # ------------------------------------------------------------------------------------------------------------------
    # Fit 2S2P1D model to the shifted data. 
    MC = Fit_MC_2S2P1D(df, ShiftCoeff)
    # Return some results. 
    return {
        'ShiftCoeff' : ShiftCoeff,
        'ReducedFreq': df['Red_Frequency'].to_numpy(),
        '2S2P1D'     : MC,
    }
# ======================================================================================================================
# ======================================================================================================================
...
def Fit_MC_2S2P1D(df, ShiftCoeff, InitialGuess=[]):
    """
    This function fits the 2S2P1D model to the already shifted datapoints. It fits to G' and G" master curves to 
    optimize the model coefficients. 

    :param df: A dataframe of the datapoints. 
    :param ShiftCoeff: A dataframe of the master curve shifting coefficients. 
    :param InitialGuess: A list of initial guesses for the model parameters. 
    :return: A dictionary of the fitted coefficients, as well as the error metrics. 
    """
    # First, define the 2S2P1D functions for the G" and G'. 
    ...
    GPP_2S2P1D = lambda w, Gg, alpha, beta, taw, k, h: Gg * (-1 * B_2S2P1D(w, alpha, beta, taw, k, h)) / \
        (((1 + A_2S2P1D(w, alpha, taw, k, h)) ** 2) + (B_2S2P1D(w, alpha, beta, taw, k, h) ** 2))
    Phase_2S2P1D = lambda w, Gg, alpha, beta, taw, k, h: np.rad2deg(np.arctan(GPP_2S2P1D(w, Gg, alpha, 
                                                                                         beta, taw, k, h) / \
    ...
    # Next, define a function to find the highest root, used for extracting parameters from the model. 
    def Find_Highest_Root(func, MaxBound, MinBound):
        MaxBound = int(np.log10(MaxBound)) + 2
        MinBound = int(np.log10(MinBound))
        root = np.nan
        Bounds = np.arange(MaxBound, MinBound, -1)
        for i in range(1, len(Bounds)):
            try: 
                result = root_scalar(func, bracket=[Bounds[i], Bounds[i - 1]], method='brentq')
                if result.converged:
                    root = 10 ** result.root
                    break
            except:
    ...
    RedFreq = df['Red_Frequency'].to_numpy()
    Gstar   = df['|G*|'].to_numpy()
    Phase   = df['PhaseAngle'].to_numpy()
    Storage = Gstar * np.cos(np.deg2rad(Phase))
    Loss    = Gstar * np.sin(np.deg2rad(Phase))
    # ------------------------------------------------------------------------------------------------------------------
    # Define the objective function, given the fixed glassy modulus of 1 GPa. 
    def ObjectiveFunction(params):
        Gg = 9
        alpha, beta, taw, k, h = params
        Val1 = np.abs(np.log10(Storage) - \
                        np.log10(GP_2S2P1D(RedFreq, 10 ** Gg, alpha, 
                                            10 ** beta, 10 ** taw, k, h))).sum() / len(RedFreq)
        Val2 = np.abs(np.log10(Loss) - \
                        np.log10(GPP_2S2P1D(RedFreq, 10 ** Gg, alpha, 
                                            10 ** beta, 10 ** taw, k, h))).sum() / len(RedFreq)
        return Val1 + Val2
    # Run the optimization. 
    result = minimize(ObjectiveFunction, x0=[2.30, np.log10(700), np.log10(2e-5), 0.21, 0.55], 
                        method='Nelder-Mead')
    # Extract the results. 
    alpha, beta, taw, k, h = result.x
    beta = 10 ** beta
    taw  = 10 ** taw
    Gg   = 10 ** 9
    # One simple check for the MAE in the Phase angle, if it was above 2°, make search a little more precise. 
    MAE = np.abs(Phase - Phase_2S2P1D(RedFreq, Gg, alpha, beta, taw, k, h)).sum() / len(RedFreq)
    iteration = 1
    while MAE > 1.5:
        warnings.warn(f'\nMean Absolute Error (MAE) in phase angle model is more than 1.5° ({MAE:.2f}° > 1.5°). ' + 
                      f'The code is refining the optimization and trying with another randomized seed value! ' + 
                      f'(Iteration {iteration}')
        # Refining the optimization search. 
        result = minimize(ObjectiveFunction, x0=[np.random.uniform(1.0, 5.0), 
                                                 np.random.uniform(1, 3.5), 
                                                 np.random.uniform(-7, -4), 
                                                 0.21, 0.55], 
    ...
    if iteration == 11:
        print(f'\tLatest solution: MAE of phase angle after {iteration} iteration = {MAE:.2f}°')
    # Calculate the model predictions. 
    RedFreq_Pred = np.logspace(-8, 5, num=100000)
    Gstar_Pred   = Gstar_2S2P1D(RedFreq_Pred, Gg, alpha, beta, taw, k, h)
    Phase_Pred   = Phase_2S2P1D(RedFreq_Pred, Gg, alpha, beta, taw, k, h)
    Storage_Pred = GP_2S2P1D(RedFreq_Pred,    Gg, alpha, beta, taw, k, h)
    Loss_Pred    = GPP_2S2P1D(RedFreq_Pred,   Gg, alpha, beta, taw, k, h)
    ObjFuncValue = result.fun
    # ----------------------------------------------------------------------------------------------------------
    # Calculate the Master curve parameters. 
    # Crossover frequency, wc, where: G'(wc) = G"(wc) -> 1 + A(wc) + B(wc) = 0
    result = root_scalar(lambda w: 1 + A_2S2P1D(10 ** w, alpha, taw, k, h) + B_2S2P1D(10 ** w, alpha, beta, taw, k, h), 
                            bracket=[-9, 9], method='bisect')
    CrossoverFreq = 10 ** result.root
    # Calculating the Rheological index. 
    R = np.log10(Gg) - np.log10(Gstar_2S2P1D(CrossoverFreq, Gg, alpha, beta, taw, k, h))
    # Calculating the |G*| at δ = 30° and δ = 45°
    w30 = Find_Highest_Root(lambda w: Phase_2S2P1D(10 ** w, Gg, alpha, beta, taw, k, h) - 30, 
                            RedFreq.max(), RedFreq.min())
    w45 = Find_Highest_Root(lambda w: Phase_2S2P1D(10 ** w, Gg, alpha, beta, taw, k, h) - 45, 
                            RedFreq.max(), RedFreq.min())
    Gstar30 = Gstar_2S2P1D(w30, Gg, alpha, beta, taw, k, h)
    Gstar45 = Gstar_2S2P1D(w45, Gg, alpha, beta, taw, k, h)
    # Calculate the temperature at which δ=30° and δ=45°, Considering 10 rad/s loading frequency. 
    aT30 = np.log10(w30) - np.log10(10)
    aT45 = np.log10(w45) - np.log10(10)
    Temp = ShiftCoeff['Temperature'].to_numpy()
    Shift= ShiftCoeff['ShiftFactor'].to_numpy()
    TempFunc = interp1d(np.flip(Shift), np.flip(Temp), kind='linear', fill_value='extrapolate')
    T30  = TempFunc(aT30)
    T45  = TempFunc(aT45)
    # Calculating the δ at |G*| = 10 MPa
    w10MPa = Find_Highest_Root(lambda w: Gstar_2S2P1D(10 ** w, Gg, alpha, beta, taw, k, h) - 1e7, 
                            RedFreq.max(), RedFreq.min())
    Phase10MPa = Phase_2S2P1D(w10MPa, Gg, alpha, beta, taw, k, h)
    # ----------------------------------------------------------------------------------------------------------
    # Store the results. 
    return {
        'Coeff': [Gg, alpha, beta, taw, k, h, 0.0],
        'Coeff_Names'   : ['Gg', 'α', 'β', 'τ', 'k', 'h', 'G0'],
        'ObjFunc_Value' : ObjFuncValue,
        'Plot_RedFreq'  : RedFreq_Pred,
        'Plot_Gstar'    : Gstar_Pred, 
        'Plot_Phase'    : Phase_Pred, 
        'Plot_Storage'  : Storage_Pred,
        'Plot_Loss'     : Loss_Pred, 
        'Crossover Frequency (rad/s)'   : CrossoverFreq,
        'Rheological Index'             : R,
        'Frequency @ δ=30° (rad/s)'     : w30, 
        'Frequency @ δ=45° (rad/s)'     : w45, 
        '|G*| @ δ=30° (Pa)'             : Gstar30, 
        '|G*| @ δ=45° (Pa)'             : Gstar45, 
        'Temperature @ δ=30° f=10 rad/s (°C)': T30, 
        'Temperature @ δ=45° f=10 rad/s (°C)': T45, 
        'Phase angle @ |G*|=10 MPa (°)' : Phase10MPa,
        }
# ======================================================================================================================
# ======================================================================================================================
# ======================================================================================================================